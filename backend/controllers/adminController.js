...
const updatePanelMembers = async (req, res) => {
  try {
    ...
    // Validate members: conveyer count, duplicates, and cross-panel conveyer uniqueness
    const validation = await validatePanelMembers(members, panelId);
    if (!validation.valid) {
      return res.status(400).json({
        success: false,
...
// ============================================
// PANEL GROUP ASSIGNMENT ENDPOINTS
// ============================================

// Get eligible groups for panel assignment
const getEligibleGroups = async (req, res) => {
  try {
    ...
    const groups = await panelGroupService.getEligibleGroupsForSemester(
      parseInt(semester),
      academicYear
    );
...
// Get all groups in a semester with assignment status breakdown
const getSemesterGroups = async (req, res) => {
  try {
    const { semester, academicYear } = req.query;
    ...
    const result = await panelGroupService.getAllGroupsInSemester(
      parseInt(semester),
      academicYear
    );
...
const autoAssignGroups = async (req, res) => {
  try {
    ...
    const result = await panelGroupService.autoAssignGroupsToPanels(
      parseInt(semester),
      academicYear
    );
...
const moveGroup = async (req, res) => {
  try {
    const { panelId, groupId } = req.params;

    if (!panelId || !groupId) {
      return res.status(400).json({
        success: false,
        message: 'panelId and groupId are required'
      });
    }

    const result = await panelGroupService.moveGroupToPanel(groupId, panelId);
...
// Deallocate a group from its panel
const deallocateGroup = async (req, res) => {
  try {
    const { groupId } = req.params;
    ...
    if (!group.panel) {
      return res.status(400).json({
        success: false,
        message: 'Group is not assigned to any panel'
      });
    }
    ...
    return res.status(200).json({
      success: true,
      message: 'Group deallocated successfully'
    });
  } catch (error) {
    return res.status(400).json({
      success: false,
...
// Get M.Tech Sem 4 Major Project registrations
const getMTechSem4Registrations = async (req, res) => {
  try {
    const { academicYear, batch, currentYear } = req.query;
    ...
    const projects = await Project.find(query)
      .populate({
        path: 'student',
        match: { degree: 'M.Tech', semester: 4 },
        populate: { path: 'user', select: 'email' }
      })
      .populate({
        path: 'faculty',
    ...
    const mtechProjects = projects.filter(project => project.student && project.student.degree === 'M.Tech' && project.student.semester === 4);
    ...
    console.error('Error getting M.Tech Sem 4 registrations:', error);
    res.status(500).json({
      success: false,
      message: 'Error fetching M.Tech Sem 4 registrations',
      error: error.message
    });
  }
};

// Get Unregistered M.Tech Sem 4 Students
const getUnregisteredMTechSem4Students = async (req, res) => {
  try {
    const { academicYear } = req.query;
    ...
    const registeredStudentIds = new Set(
      projects
        .filter(project => project.student && project.student.degree === 'M.Tech' && project.student.semester === 4)
        .map(project => project.student._id.toString())
    );
    ...
    res.json({
      ...
      data: unregisteredStudents,
      count: unregisteredStudents.length,
      message: 'Unregistered M.Tech Sem 4 students retrieved successfully'
    });
  } catch (error) {
    console.error('Error getting unregistered M.Tech Sem 4 students:', error);
    res.status(500).json({
      success: false,
      message: 'Error fetching unregistered M.Tech Sem 4 students',
      error: error.message
    });
  }
};

// Get M.Tech Sem 4 statistics for Admin Dashboard
const getMTechSem4Statistics = async (req, res) => {
  try {
    const { academicYear } = req.query;
    ...
    console.error('Error getting M.Tech Sem 4 statistics:', error);
    res.status(500).json({
      success: false,
      message: 'Error fetching M.Tech Sem 4 statistics',
      error: error.message
    });
  }
};
...
module.exports = {
  ...
  // M.Tech Sem 4 specific functions
  getMTechSem4Registrations,
  getUnregisteredMTechSem4Students,
  getMTechSem4Statistics,
