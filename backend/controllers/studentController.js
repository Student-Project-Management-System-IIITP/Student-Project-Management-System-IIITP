...
} = require("../utils/semesterMigration");
...
const getDashboardData = async (req, res) => {
  try {
    ...
    const semesterFeatures = getSemesterFeaturesData(
      student.semester,
      student.degree
    );
    ...
    semesterFeatures.availableProjects.forEach((projectType) => {
      projectCapabilities[projectType] = supportsGroupsAndFaculty(
        projectType,
        student.semester,
        student.degree
      );
    ...
    res.json({
      ...
      data: {
        ...
        stats: {
          ...
          pendingAllocations: facultyPreferences.filter(
...
const supportsGroupsAndFaculty = (projectType, semester, degree = "B.Tech") => {
  ...
  const isMtechSem1SoloWithPrefs =
    degree === "M.Tech" && semester === 1 && projectType === "minor1";

  return {
    supportsGroups:
      !soloProjects.includes(projectType) &&
      (groupProjects.includes(projectType) || semester > 5),
...
const getStudentGroups = async (req, res) => {
  try {
    ...
    const groups = await Group.find(query)
      .populate(
        "members.student",
        "fullName misNumber collegeEmail contactNumber branch"
      )
      .populate(
        "leader",
        "fullName misNumber collegeEmail contactNumber branch"
  ...
  try {
    ...
    if (semester) {
      internships = internships.filter(
        (i) => i.semester === parseInt(semester)
      );
    }
...
const registerProject = async (req, res) => {
  try {
    ...
    const projectFeatures = supportsGroupsAndFaculty(
      projectType,
      student.semester,
      student.degree
    );
    ...
    if (
      !projectFeatures.allowsMultipleProjects &&
    ...
    const projectDescription =
      projectType === "minor1" && !description ? title : description;
    ...
    if (
      student.degree === "M.Tech" &&
      student.semester === 1 &&
      projectType === "minor1"
    ) {
      // Optional limit from SystemConfig; fallback to 5
      const maxPrefs =
        (await SystemConfig.getConfigValue?.(
          "mtech.sem1.facultyPreferenceLimit",
          5
        ).catch(() => 5)) || 5;
      if (
        !facultyPreferences ||
        !Array.isArray(facultyPreferences) ||
        ...
        return res
          ...
          .json({
            success: false,
            message:
              "Faculty preferences are required for M.Tech Semester 1 registration",
          });
      }
      if (facultyPreferences.length > maxPrefs) {
        return res
          ...
          .json({
            success: false,
            message: `You can select at most ${maxPrefs} faculty preferences`,
          });
      }
...
const submitProjectFacultyPreferences = async (req, res) => {
  try {
    ...
    if (
      ...
      return res
        .status(400)
        .json({ success: false, message: "Faculty preferences are required" });
    }
    ...
    if (project.student.toString() !== student._id.toString()) {
      return res
        ...
        .json({
          success: false,
          message: "Not allowed to modify this project",
        });
    }
    ...
    if (
      ...
      return res
        ...
        .json({
          success: false,
          message: "Preferences not applicable for this project",
        });
    }
    ...
    let fpDoc = await FacultyPreference.findOne({
      project: project._id,
      student: student._id,
    ...
    res.json({
      success: true,
      message: "Faculty preferences saved",
...
const getFacultyList = async (req, res) => {
  try {
    const faculty = await Faculty.find(
      {},
      "fullName department designation mode"
    ).sort({ fullName: 1 });
...
const registerMinorProject2 = async (req, res) => {
  ...
  try {
    await session.withTransaction(async () => {
      ...
      const student = await Student.findOne({ user: studentId }).session(
        session
      );
      if (!student) {
      ...
      if (!studentAcademicYear) {
        ...
        if (anyGroup && anyGroup.academicYear) {
          ...
          studentAcademicYear = `${currentYear}-${nextYear
            .toString()
            .slice(-2)}`;
        }
      ...
      if (student.semester !== 5) {
        throw new Error(
          "Minor Project 2 is only available for Semester 5 students"
        );
      }

      // Check if student is in an active group where their membership is active
      ...
      if (!group) {
        ...
        if (anyGroup) {
          throw new Error(
            `You are in a group but academic year mismatch. Group: ${anyGroup.academicYear}, Student: ${studentAcademicYear}`
          );
        }

        throw new Error(
          "You must be in a group to register for Minor Project 2. Please create or join a group first."
        );
      }
      ...
      if (group.status !== "finalized") {
        throw new Error(
          `Your group must be finalized before registering for Minor Project 2. Current status: ${group.status}. Please finalize your group first.`
        );
      }
      ...
      if (!isGroupLeader) {
        throw new Error(
          "Only the group leader can register for Minor Project 2"
        );
      }
      ...
      const facultyPreferenceLimit = await SystemConfig.getConfigValue(
      ...
      if (
        ...
        throw new Error(
          `You must select exactly ${facultyPreferenceLimit} faculty preferences (current system requirement)`
        );
      }
      ...
      const facultyIds = facultyPreferences.map(
        (p) => p.faculty._id || p.faculty
      );
      ...
      for (const member of groupMembers) {
        const memberStudent = await Student.findById(member.student).session(
          session
        );
        if (memberStudent) {
          ...
          const existingProject = memberStudent.currentProjects.find(
      ...
      await project.populate([
        ...
        },
        {
          ...
          select: "fullName department designation mode",
        },
      ]);
...
const checkInternship1Eligibility = async (student) => {
  ...
  if (!chosenTrack) {
    return {
      eligible: false,
      reason: "Please select your track choice first.",
    };
  }
  if (chosenTrack !== "coursework") {
    return {
      eligible: false,
      reason:
        "Only students who have chosen the coursework track can register for Internship 1",
    };
  }
  ...
  if (summerApp) {
    if (["approved", "verified_pass"].includes(summerApp.status)) {
      return {
        eligible: false,
        reason:
          "You have an approved summer internship. Internship 1 is not required.",
  ...
  try {
    await session.withTransaction(async () => {
      ...
      if (!windowStatus.isOpen) {
        throw new Error(
          windowStatus.reason ||
            "Major Project 1 registration window is currently closed"
        );
      }
      ...
      if (student.semester !== 7) {
        throw new Error(
          "Major Project 1 is only available for Semester 7 students"
        );
      }
      ...
      if (!group) {
        ...
        throw new Error(
          "You must be in a group to register for Major Project 1. Please create or join a new group first."
        );
      }
      ...
      if (group.semester !== 7) {
        throw new Error(
          `Invalid group semester. Major Project 1 requires a Semester 7 group. Found: Semester ${group.semester}`
        );
      }
      ...
      if (group.status !== "finalized") {
        throw new Error(
          `Your group must be finalized before registering for Major Project 1. Current status: ${group.status}. Please finalize your group first.`
        );
      }
      ...
      if (!isGroupLeader) {
        throw new Error(
          "Only the group leader can register for Major Project 1"
        );
      }
...
const registerInternship1 = async (req, res) => {
  ...
  try {
    await session.withTransaction(async () => {
      ...
      const windowStatus = await isWindowOpen(
        "sem7.internship1.registrationWindow"
      );
      if (!windowStatus.isOpen) {
        throw new Error(
          windowStatus.reason ||
            "Internship 1 registration window is currently closed"
        );
      }
      ...
      if (student.semester !== 7) {
        throw new Error(
          "Internship 1 is only available for Semester 7 students"
        );
      }
...
const removePPT = async (req, res) => {
  try {
    ...
    const pptDeliverable = project.deliverables.find((d) =>
      d.name.toLowerCase().includes("ppt")
    );
    if (!pptDeliverable) {
      return res.status(404).json({
        success: false,
    ...
    project.deliverables = project.deliverables.filter(
      (d) => !d.name.toLowerCase().includes("ppt")
    );
    await project.save();
...
const checkSem7CourseworkEligibility = (student) => {
  ...
  if (selectedTrack !== "coursework") {
    return {
      eligible: false,
      reason: `Student is on ${selectedTrack} track, not coursework`,
    };
  }
  return { eligible: true };
};
...
const createGroup = async (req, res) => {
  try {
    ...
    if (student.semester === 5) {
      ...
      if (selectedTrack !== "coursework") {
        return res.status(400).json({
          success: false,
          message:
            "Only students finalized for coursework can form groups for Major Project 1. Please ensure your track is finalized by admin.",
        });
      }

      // Check window for Sem 7 group formation
      const windowStatus = await isWindowOpen(
        "sem7.major1.groupFormationWindow"
      );
      if (!windowStatus.isOpen) {
        return res.status(403).json({
          success: false,
          message:
            windowStatus.reason || "Group formation window is currently closed",
    ...
    // Only consider groups where this student is an ACTIVE member in an ACTIVE group
    ...
    const studentExistingMemberships = student.groupMemberships.filter(
      (gm) => gm.semester === student.semester && gm.isActive === true
    );
    ...
    if (studentExistingMemberships.length > 0) {
      ...
      student.groupMemberships = student.groupMemberships.filter(
        (gm) => !(gm.semester === student.semester && gm.isActive === true)
      );
    ...
    try {
      ...
      await student.addGroupMembershipAtomic(
        group._id,
        "leader",
        student.semester,
        session
      ...
      try {
        ...
        await cancelAllStudentInvitations(
          student._id,
          cancelSession,
          socketService,
          "Student created their own group",
          student.semester
        );
        ...
        console.error(
          "Cancel student invitations error after group creation:",
          cancelError.message
        );
...
const sendGroupInvitations = async (req, res) => {
  try {
    ...
    // For Sem 7, allow more flexible statuses since groups can transition automatically
    const allowedStatuses = group.semester === 7 
    ...
    if (!allowedStatuses.includes(group.status)) {
      return res.status(400).json({
        success: false,
        message: `Group is not in a valid status for sending invitations. Current status: ${group.status}`,
      });
    }
    ...
    try {
      ...
      for (const memberId of memberIds) {
        try {
          ...
          const invitedStudent = await Student.findById(memberId).session(
            session
          );
          if (!invitedStudent) {
          ...
          const isAlreadyMember = freshGroup.members.some(
            (member) =>
          ...
          const existingPendingInvite = freshGroup.invites.find(
            (inv) =>
          ...
          const activeMembers = freshGroup.members.filter(
          ...
          const oldRejectedInviteIndex = freshGroup.invites.findIndex(
            (inv) =>
          ...
          const oldStudentInviteIndex = invitedStudent.invites.findIndex(
            (inv) =>
              inv.group.toString() === freshGroup._id.toString() &&
              inv.status === "rejected"
          );
...
const getGroupById = async (req, res) => {
  try {
    ...
    const myInvites = group.invites.filter(
      (invite) => invite.student._id.toString() === student._id.toString()
    );
    ...
    res.json({
      ...
      data: {
        ...
        myInvites: myInvites,
        canAcceptInvites:
          group.status !== "finalized" && group.status !== "locked",
      },
...
const getAvailableStudents = async (req, res) => {
  try {
    ...
    const searchQuery = {
      _id: { $ne: student._id },
      semester: branch
        ? parseInt(semester) || student.semester
        : student.semester,
    ...
    const studentsWithStatus = await Promise.all(
      students.map(async (s) => {
        ...
        if (groupId) {
          ...
          const groupWithInvites = await Group.findById(groupId).select(
            "invites"
          );

          const hasPendingInvite = groupWithInvites?.invites?.some((invite) => {
            return (
              invite.student.toString() === s._id.toString() &&
        ...
        const finalStatus = currentGroup
          ? "in_group"
          : currentGroupInvite
          ? "pending_from_current_group"
          : hasRejectedInvite
          ? "rejected_from_current_group"
    ...
    const filteredStudentsWithStatus = studentsWithStatus.filter(
      (s) => s !== null
    );
...
const rejectAllPendingInvitations = async (
  groupId,
  session,
  socketService = null,
  ...
  for (const invite of pendingInvites) {
    ...
    if (socketService) {
      try {
        ...
        console.error(
          "Error sending auto-rejection notification:",
          notificationError
        );
      }
    }
...
const cancelAllStudentInvitations = async (
  studentId,
  session,
  socketService,
  reason = "Student joined another group",
  ...
  try {
    ...
    const groupsLedByStudent = await Group.find({
      $or: [{ createdBy: studentId }, { leader: studentId }],
    ...
    for (const group of groupsLedByStudent) {
      const sentInvites = group.invites.filter(
        (invite) =>
          invite.status === "pending" &&
    ...
    for (const group of groupsWithStudentInvites) {
      const receivedInvites = group.invites.filter(
        (invite) =>
...
const inviteToGroup = async (req, res) => {
  try {
    ...
    const member = group.members.find(
      (m) => m.student._id.toString() === student._id.toString() && m.isActive
    );

    if (
      !member ||
      (member.role !== "leader" &&
        student._id.toString() !== group.createdBy.toString())
    ) {
      return res.status(403).json({
        success: false,
    ...
    try {
      ...
      for (const invitedStudentId of studentIds) {
        try {
          ...
          const invitedStudent = await Student.findById(
            invitedStudentId
          ).session(session);
          if (!invitedStudent || invitedStudent.semester !== student.semester) {
            errors.push(
              `${invitedStudentId}: Student not found or wrong semester`
            );
            continue;
          }
          ...
          const availableSlots =
            freshGroup.maxMembers -
            freshGroup.members.filter((m) => m.isActive).length;
          ...
          const existingInvite = freshGroup.invites.find(
            (invite) =>
              invite.student.toString() === invitedStudentId &&
              invite.status === "pending"
          );

          if (existingInvite) {
            errors.push(
              `${invitedStudentId}: Student already has pending invitation`
            );
            continue;
          }

          // Check if student has a rejected invitation - if so, update it to pending
          const rejectedInvite = freshGroup.invites.find(
            (invite) =>
              invite.student.toString() === invitedStudentId &&
              invite.status === "rejected"
          );
          ...
          if (rejectedInvite) {
            ...
            if (studentRecord) {
              const studentInvite = studentRecord.invites.find(
                (invite) =>
                  invite.group.toString() === groupId &&
                  invite.status === "rejected"
              );
              if (studentInvite) {
      ...
      if (activeMembers.length >= finalGroup.maxMembers) {
        ...
        try {
          const socketService = req.app.get("socketService");
          await rejectAllPendingInvitations(
            groupId,
            rejectSession,
            socketService
          );
    ...
    try {
      ...
      if (socketService && results.length > 0) {
        for (const invitation of results) {
          ...
          if (invitedStudent) {
            await socketService.sendGroupInvitation(invitedStudent.user, {
              groupId,
...
const acceptInvitation = async (req, res) => {
  try {
    ...
    try {
      // Check if student is already in a group for this semester (only active memberships)
      ...
      const currentActiveMembers = updatedGroup.members.filter(
        (m) => m.isActive
      );
      ...
      try {
        ...
        console.error(
          "Auto-reject student invites error:",
          autoRejectError.message
        );
      ...
      try {
        ...
        if (socketService) {
          ...
          const currentMemberCount = updatedGroup.members.filter(
            (m) => m.isActive
          ).length;
      ...
      if (error.message.includes("already a member")) {
        ...
        error.message.includes("already in another group for this semester")
      ) {
        return res.status(409).json({
          success: false,
          message: "Student is already in another group for this semester",
  ...
  try {
    ...
    res.json({
      ...
      data: {
        ...
        canRegister: matchingAcademicYearGroups.some(
    ...
    if (group.semester === 7) {
      ...
      if (!windowStatus.isOpen) {
        return res.status(403).json({
          ...
          message:
            windowStatus.reason ||
            "Faculty preference submission window is currently closed",
          windowStart: windowStatus.start,
          windowEnd: windowStatus.end,
        });
      }
  ...
  try {
    ...
    res.json({
      ...
      data: {
        ...
        recommendations: getSem7Recommendations(
          internshipEligibility,
          majorProjectEligibility
        ),
      },
...
const getSem7Recommendations = (
  ...
  if (internshipEligibility.isEligible && majorProjectEligibility.isEligible) {
    ...
    recommendations.push(
      "Major project is recommended based on your eligibility"
    );
  } else {
  ...
  try {
    ...
    if (student.degree !== "M.Tech" || student.semester !== 2) {
      return res.status(400).json({
        success: false,
        message:
          "Project continuation available only for M.Tech semester 2 students",
      });
    }
  ...
  try {
    ...
    if (student.semester !== 5 && student.semester !== 7) {
      return res.status(400).json({
        success: false,
        message:
          "Group invitations are only available for semester 5 and 7 students",
      });
    }
    ...
    if (student.semester === 7) {
      ...
      if (!eligibility.eligible) {
        return res.status(400).json({
          ...
          message:
            eligibility.reason ||
            "Only coursework track students can receive group invitations",
        });
      }
    }
...
const testStudentLookup = async (req, res) => {
  try {
    ...
    const student = await Student.findById(studentId).select(
      "fullName misNumber collegeEmail branch"
    );
...
const getStudentProfile = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ success: false, message: "Error fetching student profile" });
  }
};
...
const updateStudentProfile = async (req, res) => {
  try {
    ...
    const updated = await Student.findOneAndUpdate({ user: userId }, update, {
      new: true,
      runValidators: true,
    }).populate("user", "email role isActive lastLogin");
    ...
    try {
      ...
      console.warn(
        "Warning: failed to sync User with Student profile update:",
        e?.message
      );
    }

    // Re-populate user to reflect latest
    const refreshedUser = await User.findById(updated.user._id).select(
      "email role isActive lastLogin createdAt name phone"
    );
    ...
    return res
      .status(500)
      .json({ success: false, message: "Error updating student profile" });
  }
};
  ...
  try {
    ...
    if (!sem5Membership) {
      return res.status(404).json({
        success: false,
        message:
          "No Sem 5 group found. You must have completed Semester 5 with a group.",
      });
    }
    ...
    const sem5Group = await Group.findById(sem5Membership.group)
      ...
      .populate(
        "allocatedFaculty",
        "fullName department designation email phone"
      )
    ...
    if (!sem5Group.allocatedFaculty) {
      return res.status(400).json({
        success: false,
        message:
          "Your Sem 5 group does not have an allocated faculty. Please contact admin.",
      });
    }
    ...
    const isGroupLeader =
      sem5Group.leader._id.toString() === student._id.toString();
    ...
    res.json({
      ...
      data: {
        ...
        canContinue:
          !!sem5Group.project && sem5Group.project.status !== "cancelled",
...
const registerSem6Project = async (req, res) => {
  ...
  try {
    await session.withTransaction(async () => {
      ...
      const isGroupLeader =
        sem5Group.leader.toString() === student._id.toString();
      if (!isGroupLeader) {
      ...
      if (isContinuing) {
        ...
        sem5Project = await Project.findById(previousProjectId).session(
          session
        );
        ...
        sem6Group = await migrateGroupToSem6(
          sem5Group._id,
          newAcademicYear,
          session
        );
        ...
        sem6Group = await createNewGroupForSem6(
          sem5Group._id,
          newAcademicYear,
          session
        );
        ...
        if (sem5Group.project) {
          const oldProject = await Project.findById(sem5Group.project).session(
            session
          );
          if (oldProject) {
